...
import type { InHouseResponse } from "./types/testApi.types";

function App() {
  const query = useFetch<InHouseResponse>("/testapi/inhouse");

  const data = Object.values(query.data?.data?.list || {}) || [];
                ...
                {item.empties.map(({ containers }) =>
                  containers.map((container) => (
                    <div key={container.id} className="table-row">
                      ...
                      <td headers="perdiem">{container.est_per_diem || "-"}</td>
                    </div>
                  ))
                )}
              </td>
